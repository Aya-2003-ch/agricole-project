...
<head>
    ...
    <style>
        ...
        .card.completed { border-right-color: var(--accent-green); }
        ...
        .result-item i { color: var(--accent-green); margin-left: 8px; }
...
<div class="container">
    ...
    <div class="header-section">
        <h1 class="title"><i class="fas fa-file-medical-alt"></i> إدارة الاستشارات</h1>
    ...
    <div class="card {{ $c->date_consultation ? 'completed' : 'pending' }}">
        ...
        <div class="card-header">
            <div class="farmer-info">
                <h3><i class="fas fa-user-tag"></i> الفلاح: {{ $c->user->name ?? 'غير معروف' }}</h3>
            </div>
            <div class="date-badge">
                <i class="far fa-calendar-alt"></i> طلب في: {{ $c->created_at->format('Y-m-d') }}
            </div>
        </div>

        <div class="motif-box">
            <strong><i class="fas fa-comment-medical"></i> سبب الاستشارة:</strong>
            <p style="margin: 5px 0 0 0; color: #475569;">{{ $c->motif }}</p>
        </div>
        ...
        @if(!$c->date_consultation)
            <div class="action-area">
                ...
                <button class="btn-confirm" onclick="updateConsultation({{ $c->id }})">
                    <i class="fas fa-check-circle"></i> تأكيد الحفظ والإرسال للفلاح
                </button>
            </div>
        @else
            <div class="result-box">
                <div class="result-item"><i class="fas fa-calendar-check"></i> <strong>موعد المعاينة:</strong> {{ $c->date_consultation }}</div>
                <div class="result-item"><i class="fas fa-Exclamation-circle"></i> <strong>الحالة:</strong> {{ $c->degree }}</div>
                <div class="result-item"><i class="fas fa-stethoscope"></i> <strong>التشخيص:</strong> {{ $c->diagnostique ?? 'لا يوجد تشخيص مسجل' }}</div>
            </div>
        @endif
    </div>
    @empty
        <div style="text-align: center; padding: 50px; background: white; border-radius: 20px;">
            ...
            <p style="color: #64748b; margin-top: 15px;">لا توجد استشارات مسجلة حالياً.</p>
        </div>
    @endforelse
...
function updateConsultation(id){
    ...
    fetch(`/consultation/${id}`, {
        method: "PUT", // Resource Controller يستخدم PUT للتحديث
        headers: {
    ...
    .then(data => {
        if(data.id || data.success){
            ...
            alert("❌ فشل التحديث: تأكد من إدخال البيانات بشكل صحيح");
        }
    })
    .catch(err => {
        console.error(err);
